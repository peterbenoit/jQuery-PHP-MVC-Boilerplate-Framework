...
class UserModel {
	...
	function create($info,$login = true){
		global $db;
		$name = mysql_safe($info['name']);
		$email = mysql_safe($info['email']);
		$password = md5(mysql_safe($info['password']) . PASSWORD_SALT);
		$status = $info['status'] ? mysql_safe($info['status']) : 1;
		...
		//$db->query("SELECT user_id, password FROM users WHERE email = '".mysql_safe($email)."'");
		//if(mysql_num_rows($db->result) == 1){

		if($db->num($db->query("SELECT user_id, password FROM users WHERE email = '".mysql_safe($email)."'")) > 0){
	...
	// called from controller/user.php->login
	function login($email, $password){
		global $db;
		//echo "login($email, $password)";
		$sql = $db->query("SELECT user_id, password, name FROM users WHERE email = '".mysql_safe($email)."'");
		$this->ok = false;
		if(!$email || !$password){
			...
			if($db->num($sql) == 1)
			{
				...
				if(md5($password . PASSWORD_SALT) == $db_password)
				{
					...
					$this->msg = "Error! Password is incorrect.<br />Forgot your password? <a href=\"./user/request/\">Request a new one</a>.";
				}
			}else{
				$this->msg = "Error! User does not exist.  Would you like to <a href=\"./user/register\">register</a>?";
			}
		}
		return false;
	}
	...
	function check($email, $secret){
		global $db;
		$str = "SELECT user_id, password, name FROM users WHERE email = '".mysql_safe($email)."'";
		$sql = $db->query($str);
		$results = $db->fetch($sql);
		if($db->num($sql) == 1)
		{
